...
<%
    ...
    if (userObj instanceof models.User) {
        role = ((models.User) userObj).getRole();
    } else if (userObj instanceof dto.UserDTO) {
        role = ((dto.UserDTO) userObj).getRole();
    }
    if (!"event_owner".equals(role)) {
        response.sendRedirect("login");
        return;
    }
    // Đảm bảo biến user dùng cho phía dưới
    models.User user = (userObj instanceof models.User) ? (models.User) userObj : null;
    dto.UserDTO userDTO = (userObj instanceof dto.UserDTO) ? (dto.UserDTO) userObj : null;
...
<head>
    ...
    <title>Cập nhật hồ sơ Chủ sự kiện - MasterTicket</title>
...
<header class="header">
    <nav class="nav">
        <a href="${pageContext.request.contextPath}/eventOwner/dashBoard.jsp" class="logo">MasterTicket</a>
        <div class="nav-center-content">
            <ul class="nav-links" id="navLinks">
                <li><a href="${pageContext.request.contextPath}/eventOwner/dashBoard.jsp"><i class="fas fa-home"></i> Trang chủ</a></li>
                <li><a href="${pageContext.request.contextPath}/eventOwner/createEvent/CreateEvent.jsp"><i class="fas fa-plus"></i> Tạo sự kiện</a></li>
                <li><a href="${pageContext.request.contextPath}/eventOwner/EditEventPage.jsp"><i class="fas fa-edit"></i> Quản lý sự kiện</a></li>
                <li><a href="${pageContext.request.contextPath}/eventOwner/updateEventOwnerProfile.jsp"><i class="fas fa-user"></i> Hồ sơ</a></li>
            </ul>
        </div>
            ...
            <div class="user-menu">
                <div class="user-info" onclick="toggleUserDropdown()">
                    <div class="user-avatar">
                        ...
                        <% } else { %>
                            <%= user != null && user.getEmail() != null && !user.getEmail().isEmpty() ? user.getEmail().substring(0, 1).toUpperCase() : "" %>
                        <% } %>
                    </div>
                    Xin chào, <%= user != null && user.getName() != null && !user.getName().isEmpty() ? user.getName() : user != null && user.getEmail() != null && !user.getEmail().isEmpty() ? user.getEmail() : "" %> <span style="margin-left: 0.5rem;">▼</span>
                </div>
                <div class="user-dropdown" id="userDropdown">
                    <a href="${pageContext.request.contextPath}/eventOwner/updateEventOwnerProfile.jsp" class="dropdown-item">👤 Thông tin cá nhân</a>
...
<main class="container">
    <div class="profile-header">
        <h2>Cập nhật hồ sơ chủ sự kiện</h2>
        <p>Cập nhật thông tin cá nhân của bạn</p>
    </div>
    <form method="post" action="${pageContext.request.contextPath}/updateEventOwnerProfile" enctype="multipart/form-data">
        ...
        <div class="form-grid">
            <div class="form-group">
                ...
                <input type="text" id="name" name="name" value="<%= user != null ? user.getName() : "" %>" placeholder="Tên chủ sự kiện" required>
            </div>
            <div class="form-group">
                <label for="email">Email</label>
                <input type="email" id="email" name="email" value="<%= user != null ? user.getEmail() : "" %>" placeholder="Nhập email" readonly style="opacity: 0.7;">
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <input type="date" id="birthday" name="birthday" value="<%= user != null ? dateFormat.format(user.getBirthday()) : "" %>" required>
            </div>
            <div class="form-group">
                <label for="phoneNumber">Số điện thoại</label>
                <input type="tel" id="phoneNumber" name="phoneNumber" value="<%= user != null ? user.getPhoneNumber() : "" %>" placeholder="Nhập số điện thoại" required>
            </div>
            <div class="form-group full-width">
                <label for="address">Địa chỉ</label>
                <input type="text" id="address" name="address" value="<%= user != null ? user.getAddress() : "" %>" placeholder="Nhập địa chỉ đầy đủ" required>
            </div>
        </div>
        <div class="button-group">
            <button type="submit" class="btn btn-primary">Lưu thay đổi</button>
            <a href="${pageContext.request.contextPath}/changePassword" class="btn btn-secondary">Đổi mật khẩu</a>
        </div>
...
<script>
    ...
    document.getElementById('avatar').addEventListener('change', function (event) {
        ...
        if (file) {
            ...
            var avatarUrl = '<%= user != null && user.getAvatar() != null && !user.getAvatar().isEmpty() ? (contextPath + "/uploads/user_avatar/" + user.getAvatar()) : "" %>';
            var avatarFallback = '<%= user != null && user.getEmail() != null && !user.getEmail().isEmpty() ? user.getEmail().substring(0, 1).toUpperCase() : "" %>';
            if (avatarUrl) {
